...
class PDFToolsMenuProvider(GObject.GObject, Nautilus.MenuProvider):
  ...
  def get_file_items(self, window, files):
    ...
    if files[0].get_mime_type() in ('application/pdf' 'application/x-pdf' 'application/x-bzpdf' 'application/x-gzpdf'):
      ...
      if file.exists ():
        pdf_compress = Nautilus.MenuItem(name='PDFToolsMenuProvider::pdf_compress', label='Compress scanned PDF', icon='/usr/share/icons/pdf-compress.png')
      ...
      if file.exists ():
        # ghostscript method
        pdf_repair_ghostscript = Nautilus.MenuItem(name='PDFToolsMenuProvider::pdf_repair_ghostscript', label='Repair PDF (Ghostscript)', icon='/usr/share/icons/pdf-repair.png')
        pdf_repair_ghostscript.connect('activate', self.pdf_repair_ghostscript, filename)
        submenu.append_item(pdf_repair_ghostscript)

        # mutool method
        pdf_repair_mutool = Nautilus.MenuItem(name='PDFToolsMenuProvider::pdf_repair_mutool', label='Repair PDF (MU Tool)', icon='/usr/share/icons/pdf-repair.png')
      ...
      if file.exists ():
        # rotate right
        pdf_rotate_right = Nautilus.MenuItem(name='PDFToolsMenuProvider::pdf_rotate_right', label='Rotate PDF Right (+90°)', icon='/usr/share/icons/rotate-right.png')
        pdf_rotate_right.connect('activate', self.pdf_rotate_right, filename)
        submenu.append_item(pdf_rotate_right)

        # rotate left
        pdf_rotate_left = Nautilus.MenuItem(name='PDFToolsMenuProvider::pdf_rotate_left', label='Rotate PDF Left (-90°)', icon='/usr/share/icons/rotate-left.png')
        pdf_rotate_left.connect('activate', self.pdf_rotate_left, filename)
        submenu.append_item(pdf_rotate_left)

        # rotate 180
        pdf_rotate_updown = Nautilus.MenuItem(name='PDFToolsMenuProvider::pdf_rotate_updown', label='Rotate PDF Upside Down (+180°)', icon='/usr/share/icons/rotate-updown.png')
