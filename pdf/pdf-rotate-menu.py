...
class PDFRotateMenuProvider(GObject.GObject, Nautilus.MenuProvider):
  ...
  def get_file_items(self, files):
    ...
    if onlyPDF == True and file.exists ():
      ...
      # rotate right
      pdf_rotate_right = Nautilus.MenuItem(name='PDFRotateMenuProvider::pdf_rotate_right', label='Rotate Right (+90°)', icon='/usr/share/icons/pdf-right')
      pdf_rotate_right.connect('activate', self.pdf_rotate_right, strFiles)
      submenu.append_item(pdf_rotate_right)

      # rotate left
      pdf_rotate_left = Nautilus.MenuItem(name='PDFRotateMenuProvider::pdf_rotate_left', label='Rotate Left (-90°)', icon='/usr/share/icons/pdf-left')
      pdf_rotate_left.connect('activate', self.pdf_rotate_left, strFiles)
      submenu.append_item(pdf_rotate_left)

      # rotate 180
      pdf_rotate_updown = Nautilus.MenuItem(name='PDFRotateMenuProvider::pdf_rotate_updown', label='Rotate Upside Down (+180°)', icon='/usr/share/icons/pdf-updown')
